...
class Critic2Adapter(ExternalAdapter):
    ...
    def prepare_inputs(self, context: BridgeContext) -> InputBundle:
        ...
        # 生成输入脚本
        script_lines = self._generate_script(context, density_cube)
        ...
        return InputBundle(
            ...
            auxiliary_files=[density_cube],
            command_args=[str(input_file)],
        )
    
    def _generate_script(self, context: BridgeContext, density_cube: Path) -> List[str]:
        """生成 critic2 输入脚本"""
        lines = [
        ...
        if self.analysis_type == "qtaim":
            lines.extend(self._generate_qtaim_script(density_cube))
        else:
            raise ValueError(f"Unknown analysis type: {self.analysis_type}")
        
        return lines
    
    def _generate_qtaim_script(self, density_cube: Path) -> List[str]:
        """生成 QTAIM 分析脚本"""
        lines = [
    ...
    def run_external(self, input_bundle: InputBundle) -> RunResult:
        ...
        # 执行
        exec_result = self.executor.execute_script(input_bundle.input_file)

...
def test_qcmol_alignment_redefine_items_are_frozen_to_proxy_semantics():
    ...
    assert (
        align["global_features"]["sphericity_parameters"]["mapped_path"]
...
def test_upgrade_items_mayer_and_optimized_geometry_have_frozen_proxy_status():
    ...
    assert "total_atom_count_proxy" in builder.data["global_features"]["geometry_size"]
    assert "num_bonds_proxy" in builder.data["global_features"]["geometry_size"]
    assert "num_rings_proxy" in builder.data["global_features"]["geometry_size"]
